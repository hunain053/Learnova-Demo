 ...
 */
let xpAwardsIndexEnsured = false;
async function ensureXpAwardsIndex(db) {
  if (xpAwardsIndexEnsured) return;
  await db.collection("xp_awards").createIndex(
    { firebaseUid: 1, actionType: 1, date: 1 },
    { unique: true }
  );
  xpAwardsIndexEnsured = true;
}
  ...
  while (attempt < MAX_RETRIES) {
    ...
    if (!student) {
      ...
      try {
        ...
        // Safe to ignore duplicate key errors, as we will find the document next
      }
    ...
    // Fetch true attendance history from Firestore on every attempt (no stale caching on retry)
    ...
    // Enforce idempotency: prevent duplicate XP awards for the same action on the same day
    if (actionType === "attendance_marked") {
      const alreadyAwarded = attendanceHistory.some((rec) => rec.date === todayStr);
      if (alreadyAwarded) {
        return {
          xpAwarded: 0,
      ...
      const existingAward = await db.collection("xp_awards").findOne({
        firebaseUid,
    ...
    if (actionType === "attendance_marked") {
      const attendanceInstant = metadata.attendanceDate || now;
      const streakResult = processStreak(student, attendanceInstant);
    ...
    if (actionType === "attendance_marked") {
      ...
      updateFields.lastAttendanceDate = (metadata.attendanceDate || now).toISOString();
    }
    ...
    if (updateResult.matchedCount > 0) {
      if (actionType === "attendance_marked") {
        try {
          await db.collection("xp_awards").updateOne(
            { firebaseUid, actionType: "attendance_marked", date: todayStr },
            { $setOnInsert: { firebaseUid, actionType: "attendance_marked", date: todayStr, createdAt: now } },
            { upsert: true }
          );
        } catch (_) {
          // Unique index violation on concurrent inserts is fine
        }
      }
      return {
        xpAwarded,
    ...
    // Add wider exponential jitter delay before retry: 50-250ms base + exponential scale
    const delay = 100 * Math.pow(2, attempt - 1) + Math.random() * 200 + 50;
