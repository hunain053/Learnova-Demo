...
async function ensureXpAwardsIndex(db) {
  ...
  await db
    ...
    .createIndex({ firebaseUid: 1, actionType: 1, date: 1 }, { unique: true });
    ...
    async () => {
      ...
      if (actionType === "attendance_marked") {
        try {
          ...
          console.error(
            "Failed to sync attendance history from Firestore:",
            error
          );
      ...
      const effectiveDate = metadata.attendanceDate || todayStr;

      // Enforce idempotency: prevent duplicate XP awards for the same action on the same day.
      // For offline sync, uses the attendance record's date (metadata.attendanceDate) rather than the
      // current date, so that retries across calendar days do not bypass the idempotency check.
      if (actionType === "attendance_marked") {
        const alreadyAwarded = attendanceHistory.some(
          (rec) => rec.date === effectiveDate
        );
        if (alreadyAwarded) {
          return {
            xpAwarded: 0,
      ...
      const newBadges = newlyUnlocked.filter(
        (id) => !previousBadges.includes(id)
      );
      const unlockedBadges = [
        ...new Set([...previousBadges, ...newlyUnlocked]),
      ];
      ...
      throw new Error(`Failed to update gamification state for ${firebaseUid}`);
    },
    5,
    200
  );
