...
export class AttendanceService {
  static async recordAttendance(data, decodedToken) {
    const { userId, studentName, email, confidenceScore, normalizedDate } = data;
    ...
    if (decodedToken.uid !== targetUid && isTeacherOrAdmin) {
      ...
      if (userProfile.instituteId !== callerProfile?.instituteId) {
        throw new AppError("Forbidden: Cannot submit attendance for users outside your institute", 403);
      }
      if (userProfile.role !== "student") {
        throw new AppError("Forbidden: Attendance can only be submitted for students", 403);
      }
    }
